#!/usr/bin/env npx tsx
/**
 * Import Gazette Registry from Batch Files
 * Imports gazette registry records from batch JSON files into the D1 database
 */

import { drizzle } from 'drizzle-orm/d1';
import { readFileSync, readdirSync } from 'fs';
import { join } from 'path';
import { gazetteRegistry } from '../../../src/services/database/schema';
...
interface BatchFile {
  results: Array<{
    ...
    is_extra_edition: number | boolean; // Can be 0/1 or boolean
    power: string | null;
    created_at: string;
    status: string;
...
/**
 * Import gazette registry records from a single batch file
 */
  ...
  try {
    ...
    for (const row of batch.results) {
      try {
        ...
        // Convert is_extra_edition to boolean (handle both 0/1 and boolean)
        const isExtraEdition = row.is_extra_edition === true || row.is_extra_edition === 1;

        // Insert into database
        await db.insert(gazetteRegistry).values({
          id: row.id,
...
export default {
  async run(env: any) {
    ...
    console.log('🚀 Starting Gazette Registry Import');
    console.log('═'.repeat(60));

    // Get the directory where batch files are located
    const batchDir = join(process.cwd(), 'data', 'gazette_registry');
...
if (import.meta.url === `file://${process.argv[1]}`) {
  ...
  console.log('  import importGazette from "./data/gazette_registry/import-gazette";');
  console.log('  await importGazette.run(env);');
}
