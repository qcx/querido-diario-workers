  ...
  resolveSpiderIds(
    ...
    for (const cityId of cities) {
      ...
      if (resolvedVersion === 'v2' || this.v2Registry.hasTerritoryConfigId(cityId)) {
        ...
        if (!territoryConfig) {
          territoryConfig = this.v2Registry.getTerritoryConfigByTerritoryId(cityId);
        }
        ...
        if (!territoryConfig) {
          ...
          if (territoryIdMatch) {
            ...
            territoryConfig = this.v2Registry.getTerritoryConfigByTerritoryId(ibgeId);
          }
        }
  ...
  getConfig(spiderId: string): SpiderConfig | undefined {
    ...
    if (this.v2Registry.hasTerritoryConfigId(spiderId)) {
      ...
      if (territoryConfig && territoryConfig.active) {
        ...
        const v2Spiders = this.v2Registry.getActiveSpidersForTerritory(territoryConfig.territoryId);
  ...
  createSpider(config: SpiderConfig, dateRange: DateRange, browser?: Fetcher): BaseSpider {
    switch (config.spiderType) {
      ...
      case 'prefeituracataguases':
        const cataguasesSpider = new PrefeituraCataguasesSpider(config, dateRange, browser);
      ...
      case 'diario_oficial_link':
        const diarioOficialLinkSpider = new DiarioOficialLinkSpider(config, dateRange, browser);
        if (browser) {
      ...
      case 'prefeituraguanhaes':
        const guanhaesSpider = new PrefeituraGuanhaesSpider(config, dateRange, browser);
