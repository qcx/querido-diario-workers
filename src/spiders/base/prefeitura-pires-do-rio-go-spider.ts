...
import type { PiresDoRioGoConfig } from "../../types/spider-config";

/**
 * Response type for NúcleoGov-style API (acessoainformacao.piresdorio.go.gov.br)
 */
 ...
 */
export class PrefeituraPiresDoRioGoSpider extends BaseSpider {
  protected platformConfig: PiresDoRioGoConfig;

  constructor(spiderConfig: SpiderConfig, dateRange: DateRange) {
    super(spiderConfig, dateRange);
    this.platformConfig = spiderConfig.config as PiresDoRioGoConfig;

    if (!this.platformConfig.baseUrl) {
      throw new Error(
        `PrefeituraPiresDoRioGoSpider requires baseUrl in config for ${spiderConfig.name}`,
      );
    }

    logger.info(
      `Initializing PrefeituraPiresDoRioGoSpider for ${spiderConfig.name} with URL: ${this.platformConfig.baseUrl}`,
    );
  }
  ...
  async crawl(): Promise<Gazette[]> {
    ...
    try {
      logger.debug(
        `Crawling Pires do Rio GO API: ${this.platformConfig.baseUrl} for date range ${this.dateRange.start} to ${this.dateRange.end}`,
      );
      ...
      logger.info(
        `Successfully crawled ${gazettes.length} gazettes from Pires do Rio GO`,
      );
    } catch (error) {
      logger.error(`Error crawling Pires do Rio GO:`, error as Error);
    }
