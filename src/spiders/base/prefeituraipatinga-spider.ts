...
/**
 * Configuration interface for Prefeitura Ipatinga spider
 */
...
/**
 * Spider for Prefeitura de Ipatinga diário oficial
 * 
 * Site Structure:
 ...
 */
export class PrefeituraIpatingaSpider extends BaseSpider {
  protected ipatingaConfig: PrefeituraIpatingaConfig;
  private browser: Fetcher | null = null;

  constructor(spiderConfig: SpiderConfig, dateRange: DateRange, browser?: Fetcher) {
    super(spiderConfig, dateRange);
    this.ipatingaConfig = spiderConfig.config as PrefeituraIpatingaConfig;
    this.browser = browser || null;
    
    if (!this.ipatingaConfig.baseUrl) {
      throw new Error(`PrefeituraIpatingaSpider requires a baseUrl in config for ${spiderConfig.name}`);
    }
    
    logger.info(`Initializing PrefeituraIpatingaSpider for ${spiderConfig.name} with URL: ${this.ipatingaConfig.baseUrl}`, {
  ...
  async crawl(): Promise<Gazette[]> {
    logger.info(`Crawling ${this.ipatingaConfig.baseUrl} for ${this.spiderConfig.name}...`);
    
    if (!this.browser) {
      throw new Error('PrefeituraIpatingaSpider requires browser rendering');
    }
  ...
  private async crawlWithBrowser(): Promise<Gazette[]> {
    ...
    try {
      ...
      logger.debug(`Navigating to: ${this.ipatingaConfig.baseUrl}`);
      await page.goto(this.ipatingaConfig.baseUrl, { waitUntil: 'networkidle0', timeout: 45000 });
      this.requestCount++;
      ...
      while (hasMorePages && currentPage <= maxPages) {
        ...
        // Use page.evaluate to find buttons with text content (Puppeteer doesn't support :has-text)
      ...
      for (const item of gazetteData) {
        try {
          ...
          if (!pdfUrl.startsWith('http')) {
            const baseUrlObj = new URL(this.ipatingaConfig.baseUrl);
            pdfUrl = `${baseUrlObj.origin}${pdfUrl.startsWith('/') ? '' : '/'}${pdfUrl}`;
          }
          
          // For abrir_arquivo.aspx URLs, we need to handle them specially
          // These URLs may return HTML or require server-side processing
          // So we'll create the gazette directly without URL resolution
          let gazette: Gazette | null = null;
          
          if (pdfUrl.includes('abrir_arquivo.aspx')) {
            // Create gazette directly without URL resolution for abrir_arquivo URLs
            // These URLs work but may not resolve properly with HEAD requests
            gazette = {
            ...
            // For other URLs, use the standard createGazette method
