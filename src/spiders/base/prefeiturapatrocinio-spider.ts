...
import { SpiderConfig, Gazette, DateRange, PrefeituraPatrocinioConfig } from '../../types';
import { logger } from '../../utils/logger';

/**
 * Spider for Prefeitura de Patrocínio diário oficial
 * 
 * Site Structure:
 * - URL: https://www.patrocinio.mg.gov.br/diario-eletronico
 * - Uses Portal Fácil platform similar to Ipatinga
 * - Requires browser rendering due to JavaScript-heavy page
 */
export class PrefeituraPatrocinioSpider extends BaseSpider {
  protected patrocinioConfig: PrefeituraPatrocinioConfig;
  private browser: Fetcher | null = null;

  constructor(spiderConfig: SpiderConfig, dateRange: DateRange, browser?: Fetcher) {
    super(spiderConfig, dateRange);
    this.patrocinioConfig = spiderConfig.config as PrefeituraPatrocinioConfig;
    this.browser = browser || null;
    
    if (!this.patrocinioConfig.baseUrl) {
      throw new Error(`PrefeituraPatrocinioSpider requires a baseUrl in config for ${spiderConfig.name}`);
    }
    
    logger.info(`Initializing PrefeituraPatrocinioSpider for ${spiderConfig.name} with URL: ${this.patrocinioConfig.baseUrl}`, {
  ...
  async crawl(): Promise<Gazette[]> {
    logger.info(`Crawling ${this.patrocinioConfig.baseUrl} for ${this.spiderConfig.name}...`);
    
    if (!this.browser) {
      throw new Error('PrefeituraPatrocinioSpider requires browser rendering');
    }
  ...
  private async crawlWithBrowser(): Promise<Gazette[]> {
    ...
    try {
      ...
      logger.debug(`Navigating to: ${this.patrocinioConfig.baseUrl}`);
      await page.goto(this.patrocinioConfig.baseUrl, { waitUntil: 'networkidle0', timeout: 45000 });
      this.requestCount++;
      ...
      for (const item of gazetteData) {
        try {
          ...
          if (!pdfUrl.startsWith('http')) {
            const baseUrlObj = new URL(this.patrocinioConfig.baseUrl);
