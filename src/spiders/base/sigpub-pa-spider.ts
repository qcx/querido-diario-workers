...
/**
 * SigpubPaSpider - SIGPub específico para Pará (FAMEP).
 *
 * O portal FAMEP (diariomunicipal.com.br/famep/) retorna URLs de PDF com
 * entidades HTML (&amp; em vez de &). A página principal só mostra a "última edição".
 * Para obter edições por data, usamos ?dia=DD&mes=MM&ano=YYYY para cada data no intervalo.
 *
 * Todas as prefeituras/câmaras do Pará filiadas à FAMEP publicam no mesmo portal consolidado.
 */
export class SigpubPaSpider extends BaseSpider {
  ...
  constructor(spiderConfig: SpiderConfig, dateRange: DateRange) {
    ...
    logger.info(
      `Initializing SigpubPaSpider for ${cityName} with URL: ${this.sigpubConfig.url}, entityId: ${this.sigpubConfig.entityId}`,
    );
  }
  ...
  async crawl(): Promise<Gazette[]> {
    ...
    logger.warn(
      `Note: SIGPub FAMEP uses consolidated PDFs. The gazette contains all municipalities, not just ${cityName}.`,
    );
   ...
   * FAMEP accepts ?dia=DD&mes=MM&ano=YYYY to show content for that date.
   */
  private async crawlByCalendar(): Promise<Gazette[]> {
    const gazettes: Gazette[] = [];
    const cityName = this.sigpubConfig.cityName || this.spiderConfig.name;
    const entityId = this.sigpubConfig.entityId;
    const baseUrl = this.sigpubConfig.url
      .replace(/\?.*$/, "")
      .replace(/\/$/, "");
    ...
    while (currentDate <= endDate) {
      ...
      try {
        logger.debug(`Fetching FAMEP for ${dateStr}: ${url}`);
        ...
        for (const match of matches) {
          ...
          if (gazette) {
            gazette.notes = `Consolidated gazette from FAMEP. EntityId for ${cityName}: ${entityId}`;
  ...
  private async crawlMainPageFallback(): Promise<Gazette[]> {
    ...
    if (gazettes.length > 0) {
      logger.info(
        `Found ${gazettes.length} gazettes for ${cityName} from main page fallback`,
      );
    }
    return gazettes;
  }
