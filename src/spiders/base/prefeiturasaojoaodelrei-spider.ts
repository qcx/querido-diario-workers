...
import { SpiderConfig, Gazette, DateRange, PrefeituraSaoJoaoDelReiConfig } from '../../types';
import { logger } from '../../utils/logger';

/**
 * Spider for Prefeitura de São João del-Rei diário oficial
 * 
 * Site Structure:
 * - URL: https://www.saojoaodelrei.mg.gov.br/diario-eletronico
 * - Uses Portal Fácil platform similar to Ipatinga
 * - Requires browser rendering due to JavaScript-heavy page
 */
export class PrefeituraSaoJoaoDelReiSpider extends BaseSpider {
  protected saojoaodelreiConfig: PrefeituraSaoJoaoDelReiConfig;
  private browser: Fetcher | null = null;

  constructor(spiderConfig: SpiderConfig, dateRange: DateRange, browser?: Fetcher) {
    super(spiderConfig, dateRange);
    this.saojoaodelreiConfig = spiderConfig.config as PrefeituraSaoJoaoDelReiConfig;
    this.browser = browser || null;
    
    if (!this.saojoaodelreiConfig.baseUrl) {
      throw new Error(`PrefeituraSaoJoaoDelReiSpider requires a baseUrl in config for ${spiderConfig.name}`);
    }
    
    logger.info(`Initializing PrefeituraSaoJoaoDelReiSpider for ${spiderConfig.name} with URL: ${this.saojoaodelreiConfig.baseUrl}`, {
  ...
  async crawl(): Promise<Gazette[]> {
    logger.info(`Crawling ${this.saojoaodelreiConfig.baseUrl} for ${this.spiderConfig.name}...`);
    
    if (!this.browser) {
      throw new Error('PrefeituraSaoJoaoDelReiSpider requires browser rendering');
    }
  ...
  private async crawlWithBrowser(): Promise<Gazette[]> {
    ...
    try {
      ...
      logger.debug(`Navigating to: ${this.saojoaodelreiConfig.baseUrl}`);
      await page.goto(this.saojoaodelreiConfig.baseUrl, { waitUntil: 'networkidle0', timeout: 45000 });
      this.requestCount++;
      ...
      for (const item of gazetteData) {
        try {
          ...
          if (!pdfUrl.startsWith('http')) {
            const baseUrlObj = new URL(this.saojoaodelreiConfig.baseUrl);
