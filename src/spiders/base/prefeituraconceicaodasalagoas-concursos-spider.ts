...
import { SpiderConfig, Gazette, DateRange, PrefeituraConceicaoDasAlagoasConcursosConfig } from '../../types';
import { logger } from '../../utils/logger';
// @ts-ignore
import { parse } from 'node-html-parser';

/**
 * Spider for Prefeitura de Conceição das Alagoas - Concursos
 * 
 * Site Structure:
 ...
 * - Each post has a title (e.g., "Decreto Municipal 253/2025 – Nomeação em Cargo Público Efetivo")
 * - Date format: "abril 2, 2025" (month day, year)
 * - PDF links: "Baixar arquivo" and "Visualizar arquivo" buttons
 */
export class PrefeituraConceicaoDasAlagoasConcursosSpider extends BaseSpider {
  protected concursosConfig: PrefeituraConceicaoDasAlagoasConcursosConfig;
  private browser: Fetcher | null = null;

  constructor(spiderConfig: SpiderConfig, dateRange: DateRange, browser?: Fetcher) {
    super(spiderConfig, dateRange);
    this.concursosConfig = spiderConfig.config as PrefeituraConceicaoDasAlagoasConcursosConfig;
    this.browser = browser || null;
    
    if (!this.concursosConfig.baseUrl) {
      throw new Error(`PrefeituraConceicaoDasAlagoasConcursosSpider requires a baseUrl in config for ${spiderConfig.name}`);
    }
    
    logger.info(`Initializing PrefeituraConceicaoDasAlagoasConcursosSpider for ${spiderConfig.name} with URL: ${this.concursosConfig.baseUrl}`, {
  ...
  async crawl(): Promise<Gazette[]> {
    logger.info(`Crawling ${this.concursosConfig.baseUrl} for ${this.spiderConfig.name}...`);
    
    // Use browser-based crawling if browser is available and requiresClientRendering is true
    if (this.browser && this.concursosConfig.requiresClientRendering === true) {
    ...
    try {
      ...
      while (hasMorePages) {
        const pageUrl = currentPage === 1 
          ? this.concursosConfig.baseUrl 
          : `${this.concursosConfig.baseUrl}page/${currentPage}/`;
        ...
        for (const post of posts) {
          try {
            ...
            // Pattern 1: "abril 2, 2025" or "abril 2 2025"
            ...
            if (!pdfUrl.startsWith('http')) {
              const baseUrlObj = new URL(this.concursosConfig.baseUrl);
              pdfUrl = `${baseUrlObj.origin}${pdfUrl.startsWith('/') ? '' : '/'}${pdfUrl}`;
            }

            // Extract edition number from title if possible (e.g., "Decreto Municipal 253/2025")
            const editionMatch = title.match(/(\d+)\/(\d{4})/);
  ...
  private async crawlWithBrowser(): Promise<Gazette[]> {
    ...
    try {
      ...
      logger.debug(`Navigating to: ${this.concursosConfig.baseUrl}`);
      await page.goto(this.concursosConfig.baseUrl, { waitUntil: 'networkidle0', timeout: 45000 });
      this.requestCount++;
    ...
    try {
      ...
      for (const item of gazetteData) {
        try {
          ...
          const editionMatch = item.title.match(/(\d+)\/(\d{4})/);
          const editionNumber = editionMatch ? editionMatch[1] : undefined;
