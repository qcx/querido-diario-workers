...
import { SpiderConfig, Gazette, DateRange, ApreceConfig } from '../../types';
import { logger } from '../../utils/logger';
import { toISODate } from '../../utils/date-utils';

/**
 * Spider para o consórcio APRECE (Associação dos Municípios do Estado do Ceará)
 * 
 * Este spider coleta diários oficiais do sistema centralizado da APRECE
 * através do portal SIGPub da Vox Tecnologia.
 * 
 * URL Base: https://www.diariomunicipal.com.br/aprece/
 * Pesquisa: https://www.diariomunicipal.com.br/aprece/pesquisar
 * 
 * Características:
 * - Sistema SIGPub da Vox Tecnologia
 * - Sistema centralizado estadual (APRECE)
 * - Municípios do CE publicam no mesmo sistema
 * - Download direto via URL do storage
 * - Filtro por entidade (município)
 * 
 * Municípios participantes incluem (conforme dropdown do site):
 * - Prefeitura Municipal de Iguatu
 * - Prefeitura Municipal de Quixadá
 * - Prefeitura Municipal de Quixeramobim
 * - E muitos outros municípios do Ceará
 */
export class ApreceSpider extends BaseSpider {
  protected apreceConfig: ApreceConfig;
  private readonly BASE_URL = 'https://www.diariomunicipal.com.br/aprece';

  constructor(spiderConfig: SpiderConfig, dateRange: DateRange) {
    super(spiderConfig, dateRange);
    this.apreceConfig = spiderConfig.config as ApreceConfig;
    
    logger.info(`Initializing ApreceSpider for ${spiderConfig.name} (APRECE) - Entity: ${this.apreceConfig.entityName}`);
  }

  async crawl(): Promise<Gazette[]> {
    logger.info(`Crawling APRECE (CE) for ${this.spiderConfig.name}...`);
        ...
        for (const match of matches) {
          ...
          if (pdfDateStr === dateStr && !foundUrls.has(pdfUrl)) {
            ...
            const gazette = await this.createGazette(date, pdfUrl, {
              ...
              power: this.apreceConfig.power || 'executive_legislative',
              sourceText: `APRECE - ${this.apreceConfig.entityName}`
            });
  ...
  async getApiInfo(): Promise<any> {
    try {
      ...
      return {
        ...
        entityName: this.apreceConfig.entityName,
