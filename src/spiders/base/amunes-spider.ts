...
import { SpiderConfig, Gazette, DateRange, AmunesConfig } from '../../types';
import { logger } from '../../utils/logger';
import { toISODate } from '../../utils/date-utils';

/**
 * Spider para o consórcio AMUNES (Associação dos Municípios do Espírito Santo)
 * 
 * Este spider coleta diários oficiais do sistema centralizado da AMUNES
 * através de API estruturada.
 * 
 ...
 * - Municípios do ES publicam no mesmo sistema
 * - Download direto por ID da edição
 * - Informações completas: número, páginas, data, etc.
 */
export class AmunesSpider extends BaseSpider {
  protected amunesConfig: AmunesConfig;
  ...
  constructor(spiderConfig: SpiderConfig, dateRange: DateRange) {
    ...
    this.amunesConfig = spiderConfig.config as AmunesConfig;
    
    logger.info(`Initializing AmunesSpider for ${spiderConfig.name} (AMUNES)`);
  }

  async crawl(): Promise<Gazette[]> {
    logger.info(`Crawling AMUNES (ES) for ${this.spiderConfig.name}...`);
  ...
  private async createGazetteFromItem(item: any, dateStr: string): Promise<Gazette | null> {
    try {
      ...
      return await this.createGazette(date, downloadUrl, {
        ...
        power: this.amunesConfig.power || 'executive_legislative',
