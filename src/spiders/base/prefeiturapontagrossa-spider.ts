...
export class PrefeiturapontagrossaSpider extends BaseSpider {
  ...
  constructor(config: SpiderConfig, dateRange: DateRange) {
    ...
    this.gedApiUrl = platformConfig.gedApiUrl || 'https://pontagrossa.oxy.elotech.com.br/ged-api/api/file/get-file-content';
  }
  ...
  async crawl(): Promise<Gazette[]> {
    ...
    try {
      // The Elotech OXY platform is a React SPA
      // Try the API endpoint for listing gazettes
      ...
      // If API didn't work, try to fetch the HTML page and look for GED URLs
      if (gazettes.length === 0) {
        ...
        if (htmlResponse.ok) {
          ...
          while ((match = gedRegex.exec(html)) !== null) {
            ...
            // Extract date from key pattern: diario-oficial-ordinaria-XXXX-DD-MM-YYYY
