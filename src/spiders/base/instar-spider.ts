...
export class InstarSpider extends BaseSpider {
  ...
  constructor(
    ...
    if (!this.instarConfig.url) {
      throw new Error(
        `InstarSpider requires a base_url in config for ${spiderConfig.name}`,
      );
    }

    logger.info(
      `Initializing InstarSpider for ${spiderConfig.name} with URL: ${this.instarConfig.url}`,
      {
  ...
  async crawl(): Promise<Gazette[]> {
    logger.info(
      `Crawling ${this.instarConfig.url} for ${this.spiderConfig.name}...`,
    );
      ...
      while (hasMorePages) {
        ...
        try {
          await page.waitForSelector(
            ".edocman-document-title-td, table tbody tr, .dof_publicacao_diario, article",
            { timeout: 10000 },
          );
        } catch (error) {
        ...
        if (
          ...
          (hasEdocmanSpecific || hasEdocmanTable)
        ) {
        ...
        logger.debug(
          `Found ${pageGazettes.length} gazettes on page ${currentPage}, ${gazettes.length} in date range`,
        );
        ...
        const nextPageButton = await page.$(
          'a[href*="page"], .pagination .next:not(.disabled), .pager .next:not(.disabled), button[name="Anteriores"], [class*="pagination"] button:not(:disabled)',
        );
  ...
  private async crawlWithFetch(): Promise<Gazette[]> {
    ...
    try {
      ...
      const articleElements = basePageRoot.querySelectorAll(
        "article.list-item, article",
      );
      if (articleElements.length > 0) {
        logger.info(
          `Detected static article-based format with ${articleElements.length} articles`,
        );
        return this.crawlStaticArticleFormat(basePageHtml);
      }

      // Check if it has Instar URL pattern support (sw_qtde_resultados indicator)
      const hasInstarPattern =
        basePageRoot.querySelector(".sw_qtde_resultados") !== null;
      ...
      if (!hasInstarPattern) {
        ...
        try {
          ...
          if (!testResults) {
            logger.warn(
              `Site ${this.spiderConfig.name} does not support standard Instar URL pattern, trying article format`,
            );
            return this.crawlStaticArticleFormat(basePageHtml);
          }
        } catch (error) {
          logger.warn(
            `Could not access Instar URL pattern, falling back to article format`,
          );
          return this.crawlStaticArticleFormat(basePageHtml);
        }
      }
   ...
   * </article>
   */
  private async crawlStaticArticleFormat(
    ...
    while (pageNum <= maxPages && !foundOlderThanRange) {
      ...
      for (const article of articles) {
        try {
          ...
          const titleElement = article.querySelector(
            ".list-item__title, h1, h2, h3, h4, h5, h6",
          );
          const titleText = titleElement?.text?.trim() || "";

          // Extract date
          const dateElement = article.querySelector(
            ".list-item__description, p, time",
          );
          ...
          const pdfLink = article.querySelector(
            'a[href*=".pdf"], a[href*="ecrie"], a[href*="download"]',
          );
          ...
          const editionMatch =
            titleText.match(/[Ee]di[çc][ãa]o\s+[nN]?[°º]?\s*(\d+)/i) ||
            titleText.match(/(\d+)/);
          const editionNumber = editionMatch ? editionMatch[1] : undefined;

          // Check if extra edition
          const isExtraEdition =
            titleText.toLowerCase().includes("extra") ||
      ...
      logger.debug(
        `Found ${gazettes.length} gazettes so far after page ${pageNum}`,
      );
      ...
      const paginationSelect = root.querySelector(
        'select.pagination__select, select[name*="page"], select[aria-label*="página"], select[class*="page"]',
      );
      const nextPageLink = root.querySelector(
        'a[href*="page="], a[href*="pagina="], a[href*="/page/"], a[rel="next"], button[name="Anteriores"]',
      );
      ...
      if (paginationSelect) {
        ...
        for (const option of options) {
          ...
          if (foundCurrent && value) {
            ...
            const usesPagina =
              this.instarConfig.url.includes("pagina=") ||
      ...
      try {
        ...
        const nextArticles = nextRoot.querySelectorAll(
          "article.list-item, article",
        );
        ...
        const firstCurrentTitle = articles[0]
          ?.querySelector(".list-item__title, h1, h2, h3")
          ?.text?.trim();
        const firstNextTitle = nextArticles[0]
          ?.querySelector(".list-item__title, h1, h2, h3")
          ?.text?.trim();
        if (
          firstCurrentTitle &&
          firstNextTitle &&
          firstCurrentTitle === firstNextTitle
        ) {
        ...
        logger.debug(
          `Error fetching page ${pageNum + 1}, stopping pagination: ${error}`,
        );
        break;
      }
    }
    ...
    logger.info(
      `Successfully crawled ${gazettes.length} gazettes using static article format`,
    );
    return gazettes;
  }
    ...
    try {
      ...
      const documentRows = await page.evaluate(() => {
        ...
        for (const row of Array.from(tableRows)) {
          ...
          const titleTd = row.querySelector(
            ".edocman-document-title-td, td.edocman-document-title-td",
          );
          const dateInfo = row.querySelector(
            '.dateinformation, [class*="dateinformation"], td.dateinformation',
          );
          ...
          const tableView = row.querySelector(
            '.documents_table_view, [class*="documents_table_view"], td.documents_table_view',
          );
        ...
        try {
          ...
          if (row.pdfHref && row.hasTableView) {
            ...
            const pdfLink = await page.$(
              ".edocmandownloadlink.btn.btn-secondary, a.edocmandownloadlink",
            );
            if (pdfLink) {
          ...
          if (!pdfUrl) {
            logger.warn(
              `Could not find PDF URL for document: ${row.titleText}`,
            );
            continue;
          }
          ...
          const editionMatch = row.titleText?.match(
          ...
          const gazette = await this.createGazette(gazetteDate, pdfUrl, {
            editionNumber,
            isExtraEdition:
              row.titleText?.toLowerCase().includes("extra") || false,
            power: "executive_legislative",
    ...
    try {
      ...
      const gazetteElements = await page.evaluate(() => {
        ...
        for (const container of Array.from(containers)) {
          ...
          const downloadHref = downloadElement
            ? downloadElement.getAttribute("data-href")
            : null;
      ...
      logger.debug(
        `Found ${gazetteElements.length} standard Instar gazette elements on page`,
      );
      ...
      for (const element of gazetteElements) {
        try {
          ...
          let gazetteDate: Date | null = null;
          if (element.dateText) {
            const dateMatch = element.dateText.match(
              /(\d{2})\/(\d{2})\/(\d{4})/,
            );
            if (dateMatch) {
              const [, day, month, year] = dateMatch;
          ...
          if (!element.downloadHref) {
            logger.warn(
              `No download link found for gazette: ${element.titleText}`,
            );
            continue;
          }
          ...
          // Extract edition number
          const editionMatch = element.titleText?.match(
            /[Ee]di[çc][ãa]o\s+[nN]?[°º]?\s*(\d+)/i,
          );
          const editionNumber = editionMatch ? editionMatch[1] : undefined;
          ...
          const isExtraEdition =
            element.titleText?.toLowerCase().includes("extra") || false;
          ...
          const gazette = await this.createGazette(gazetteDate, pdfUrl, {
            ...
            power: "executive_legislative",
            sourceText:
              element.titleText || `Gazette ${toISODate(gazetteDate)}`,
          });
          ...
          logger.error(
            `Error processing standard Instar gazette element:`,
            error as Error,
          );
        }
      }
      ...
      logger.error(
        `Error extracting standard Instar gazettes from page:`,
        error as Error,
      );
    }
    ...
    try {
      ...
      const articleElements = await page.evaluate(() => {
        ...
        for (const article of Array.from(articles)) {
          ...
          const headingElement = article.querySelector(
            'h1, h2, h3, h4, h5, h6, [class*="heading"]',
          );
          const titleText = headingElement
          ...
          const downloadLink = article.querySelector(
            'a[href*=".pdf"], a[href*="download"], a',
          );
          let downloadHref = downloadLink
          ...
          if (
            downloadHref &&
            !downloadText?.toLowerCase().includes("download") &&
            !downloadHref.includes(".pdf") &&
            !downloadHref.includes("download")
          ) {
      ...
      logger.debug(
        `Found ${articleElements.length} article-based gazette elements on page`,
      );
      ...
      for (const element of articleElements) {
        try {
          ...
          if (!gazetteDate) {
            logger.debug(
              `Could not parse date from article: ${element.titleText}`,
            );
            continue;
          }
          ...
          if (!element.downloadHref) {
            logger.warn(
              `No download link found for article gazette: ${element.titleText}`,
            );
            continue;
          }
          ...
          logger.error(
            `Error processing article-based gazette element:`,
            error as Error,
          );
        }
      }
      ...
      logger.error(
        `Error extracting article-based gazettes from page:`,
        error as Error,
      );
    }
   ...
   * </div>
   */
  private async parseGazetteElement(
    gazetteElement: any,
  ): Promise<Gazette | null> {
    try {
      // Extract edition number from title
      const titleElement = gazetteElement.querySelector(
        ".dof_titulo_publicacao span",
      );
      ...
      if (!gazetteDate) {
        logger.warn(
          `Could not extract date from gazette element: ${titleText}`,
        );
        return null;
      }
      ...
      if (!downloadHref) {
        logger.warn(
          `No download link found for gazette ${titleText} on ${gazetteDate.toISOString().split("T")[0]}`,
        );
        return null;
      }
