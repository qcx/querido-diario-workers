...
import { SpiderConfig, Gazette, DateRange, PrefeituraManhuacuConfig } from '../../types';
import { logger } from '../../utils/logger';

/**
 * Spider for Prefeitura de Manhuaçu diário oficial
 * 
 * Site Structure:
 * - URL: https://www.manhuacu.mg.gov.br/diario-eletronico
 * - Uses Portal Fácil platform similar to Ipatinga
 * - Requires browser rendering due to JavaScript-heavy page
 */
export class PrefeituraManhuacuSpider extends BaseSpider {
  protected manhuacuConfig: PrefeituraManhuacuConfig;
  private browser: Fetcher | null = null;

  constructor(spiderConfig: SpiderConfig, dateRange: DateRange, browser?: Fetcher) {
    super(spiderConfig, dateRange);
    this.manhuacuConfig = spiderConfig.config as PrefeituraManhuacuConfig;
    this.browser = browser || null;
    
    if (!this.manhuacuConfig.baseUrl) {
      throw new Error(`PrefeituraManhuacuSpider requires a baseUrl in config for ${spiderConfig.name}`);
    }
    
    logger.info(`Initializing PrefeituraManhuacuSpider for ${spiderConfig.name} with URL: ${this.manhuacuConfig.baseUrl}`, {
  ...
  async crawl(): Promise<Gazette[]> {
    logger.info(`Crawling ${this.manhuacuConfig.baseUrl} for ${this.spiderConfig.name}...`);
    
    if (!this.browser) {
      throw new Error('PrefeituraManhuacuSpider requires browser rendering');
    }
  ...
  private async crawlWithBrowser(): Promise<Gazette[]> {
    ...
    try {
      ...
      logger.debug(`Navigating to: ${this.manhuacuConfig.baseUrl}`);
      await page.goto(this.manhuacuConfig.baseUrl, { waitUntil: 'networkidle0', timeout: 45000 });
      this.requestCount++;
      ...
      for (const item of gazetteData) {
        try {
          ...
          if (!pdfUrl.startsWith('http')) {
            const baseUrlObj = new URL(this.manhuacuConfig.baseUrl);
