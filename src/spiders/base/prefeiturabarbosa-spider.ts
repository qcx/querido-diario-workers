...
import { SpiderConfig, Gazette, DateRange, PrefeituraBarbosaConfig } from '../../types';
import { logger } from '../../utils/logger';
import { toISODate } from '../../utils/date-utils';

/**
 * PrefeituraBarbosaSpider implementation
 * 
 * Crawls Barbosa's official gazette website which uses date range URLs
 ...
 */
export class PrefeituraBarbosaSpider extends BaseSpider {
  protected barbosaConfig: PrefeituraBarbosaConfig;

  constructor(spiderConfig: SpiderConfig, dateRange: DateRange) {
    super(spiderConfig, dateRange);
    this.barbosaConfig = spiderConfig.config as PrefeituraBarbosaConfig;
    
    if (!this.barbosaConfig.baseUrl) {
      throw new Error(`PrefeituraBarbosaSpider requires baseUrl in config for ${spiderConfig.name}`);
    }
    
    logger.info(`Initializing PrefeituraBarbosaSpider for ${spiderConfig.name}`);
  }

  async crawl(): Promise<Gazette[]> {
    logger.info(`Crawling ${this.barbosaConfig.baseUrl} for ${this.spiderConfig.name}...`);
    ...
    try {
      ...
      const searchUrl = `${this.barbosaConfig.baseUrl}/1/${startDateStr}/${endDateStr}/0/0/`;
      ...
      gazetteItems.each((_, element) => {
        try {
          ...
          if (pdfPath.startsWith('http')) {
            ...
            // Extract base domain from baseUrl (e.g., "https://www.barbosa.sp.gov.br")
            const baseUrlObj = new URL(this.barbosaConfig.baseUrl);
