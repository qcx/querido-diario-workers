...
export class PrefeituracuritibaSpider extends BaseSpider {
  ...
  private async crawlWithBrowser(): Promise<Gazette[]> {
    ...
    try {
      ...
      while (currentDate <= endDate) {
        ...
        logger.info(`Fetching Curitiba gazettes for ${year}/${month} (browser)`);
        ...
        for (const row of rows) {
          ...
          const pdfUrl = await this.clickVisualizarAndGetPdfUrl(page, row.linkId);
          if (!pdfUrl) {
            logger.warn(`Could not get PDF URL for Curitiba ${row.dateStr} ed. ${row.editionNumber}`);
            continue;
          }
      ...
      if (browserInstance) try { await browserInstance.close(); } catch (e) { /* ignore */ }
    }
    return gazettes;
  }

  private async clickVisualizarAndGetPdfUrl(page: any, linkId: string): Promise<string | null> {
    const baseOrigin = new URL(this.baseUrl).origin;

    const pdfPromise = page.waitForResponse(
      (res: { url: () => string; status: () => number; headers: () => Record<string, string> }) => {
        const url = res.url();
        const ct = (res.headers() || {})['content-type'] || '';
        return (
          res.status() === 200 &&
          (url.includes('.pdf') || ct.includes('application/pdf') || ct.includes('application/octet-stream'))
        );
      },
      { timeout: 15000 },
    ).catch(() => null);

    const navPromise = page.waitForNavigation({ waitUntil: 'load', timeout: 15000 }).catch(() => null);
      ...
      if (currentUrl.includes('.pdf')) {
        ...
        if (iframeMatch) {
          ...
          if (src && (src.includes('.pdf') || src.includes('pdf'))) {
            result = src.startsWith('http') ? src : `${baseOrigin}${src.startsWith('/') ? '' : '/'}${src}`;
          }
        }
        if (!result) {
          ...
          if (objectMatch) {
            ...
            if (data) result = data.startsWith('http') ? data : `${baseOrigin}${data.startsWith('/') ? '' : '/'}${data}`;
          }
        }
        if (!result) {
          ...
          if (pdfLinkMatch) {
            ...
            if (href) result = href.startsWith('http') ? href : `${baseOrigin}${href.startsWith('/') ? '' : '/'}${href}`;
          }
        }
        if (!result && (currentUrl.toLowerCase().includes('exibir') || currentUrl.toLowerCase().includes('visualizar'))) {
          result = currentUrl;
        }
      }
      if (result) {
  ...
  private async crawlWithFetch(): Promise<Gazette[]> {
    const gazettes: Gazette[] = [];
    logger.info(`Crawling Curitiba Legisladoc for ${this.config.name} (fetch – use browser for real PDFs)...`);
      ...
      while (currentDate <= endDate) {
        ...
        const searchResponse = await fetch(this.baseUrl, {
          ...
          headers: {
            ...
            ...(response.headers.get('set-cookie') && { Cookie: response.headers.get('set-cookie') ?? '' }),
          },
        ...
        const sessionCookie = searchResponse.headers.get('set-cookie') ?? response.headers.get('set-cookie') ?? '';
        const searchViewStateMatch = searchHtml.match(/id="__VIEWSTATE"\s+value="([^"]*)"/);
        const searchEventValidationMatch = searchHtml.match(/id="__EVENTVALIDATION"\s+value="([^"]*)"/);
        ...
        while ((match = rowRegex.exec(searchHtml)) !== null) {
          rows.push({ editionNumber: match[1].trim(), dateStr: match[2], eventTarget: match[3] });
        }

        const viewState = searchViewStateMatch?.[1] ?? viewStateMatch[1];
        const eventValidation = searchEventValidationMatch?.[1] ?? eventValidationMatch[1];
        const viewStateGenerator = searchViewStateGeneratorMatch?.[1] ?? viewStateGeneratorMatch?.[1];
        ...
        if (gazettes.length === 0 && rows.length === 0) {
          ...
          while ((dateMatch = simpleDateRegex.exec(searchHtml)) !== null) {
            ...
            if (isoDate >= this.dateRange.start && isoDate <= this.dateRange.end && !dates.has(isoDate)) {
              dates.add(isoDate);
              gazettes.push({
                date: isoDate,
  ...
  private async getGazetteUrlFromPostback(
    ...
    if (finalUrl && finalUrl !== this.baseUrl && (contentType.includes('application/pdf') || finalUrl.toLowerCase().includes('.pdf')))
      return finalUrl;
    ...
    if (iframeMatch) {
      const src = iframeMatch[1].trim();
      if (src && (src.includes('.pdf') || src.includes('pdf') || src.includes('Visualizar') || src.includes('viewer')))
        return src.startsWith('http') ? src : `${baseOrigin}${src.startsWith('/') ? '' : '/'}${src}`;
    }
    const objectMatch = html.match(/<object[^>]+data=["']([^"']+)["']/i);
    if (objectMatch) {
      const data = objectMatch[1].trim();
      if (data) return data.startsWith('http') ? data : `${baseOrigin}${data.startsWith('/') ? '' : '/'}${data}`;
    }
    const pdfLinkMatch = html.match(/<a[^>]+href=["']([^"']*\.pdf[^"']*)["']/i) ?? html.match(/href=["']([^"']*(?:download|getfile|pdf)[^"']*)["']/i);
    if (pdfLinkMatch) {
      const href = pdfLinkMatch[1].trim();
      if (href) return href.startsWith('http') ? href : `${baseOrigin}${href.startsWith('/') ? '' : '/'}${href}`;
    }
