...
/**
 * Spider for sites that display gazettes as links with "Diário Oficial" text
 * Used by municipalities like Jacutinga-MG
 * 
 * HTML Structure:
 * <a>Diário Oficial 2871 - DOEM - 12 de Janeiro de 2026 Data: 12/01/2026 Edição: 2871</a>
 * <form>
 *   <input name="arquivo_2871 - DOEM - 12 de Janeiro de 2026.pdf">
 *   <input name="pdf_diario_oficial">
 * </form>
 */
export class DiarioOficialLinkSpider extends BaseSpider {
  ...
  constructor(spiderConfig: SpiderConfig, dateRange: DateRange, browser?: Fetcher) {
    ...
    if (!this.config.url) {
      throw new Error(`DiarioOficialLinkSpider requires a url in config for ${spiderConfig.name}`);
    }
    
    if (!this.browser) {
      throw new Error(`DiarioOficialLinkSpider requires browser for ${spiderConfig.name}`);
    }
    
    logger.info(`Initializing DiarioOficialLinkSpider for ${spiderConfig.name} with URL: ${this.config.url}`);
  }
  ...
  async crawl(): Promise<Gazette[]> {
    ...
    if (!this.browser) {
      throw new Error('Browser is required for DiarioOficialLinkSpider');
    }
  ...
  /**
   * Browser-based crawling for sites with link-based format
   */
    ...
    try {
        ...
        for (const link of Array.from(links)) {
          ...
          if (text.toLowerCase().includes('diário oficial') || text.toLowerCase().includes('diario oficial')) {
            ...
            if (!pdfUrl || (!pdfUrl.includes('.pdf') && !pdfUrl.includes('download'))) {
              ...
              if (!foundForm || !pdfUrl || (!pdfUrl.includes('.pdf') && !pdfUrl.includes('download'))) {
                ...
                for (let i = 0; i < 3 && container; i++) {
                  ...
                  for (const downloadLink of Array.from(downloadLinks)) {
                    ...
                    // Check if URL or filename contains diario/diário/oficial/edicao/edição or edition number
              ...
              // Final validation: reject PDFs that don't look like diário oficial
              if (pdfUrl) {
      ...
      for (const element of linkElements) {
        try {
          ...
          // This is the most reliable method for sites like Jacutinga
          if (!pdfUrl || !pdfUrl.includes('.pdf')) {
            ...
            try {
              ...
              // Validate that it's a diário oficial PDF
              const pdfLinkOnPage = await page.evaluate((editionNum) => {
                ...
                // Strategy 4: Look for PDF in page content (sometimes PDFs are embedded)
                const pageContent = document.documentElement.innerHTML;
                const pdfMatches = pageContent.match(/['"]([^'"]*\.pdf[^'"]*)['"]/gi);
                if (pdfMatches) {
                  for (const match of pdfMatches) {
                    const pdfUrl = match.replace(/['"]/g, '');
                    const urlLower = pdfUrl.toLowerCase();
                    const hasDiarioKeywords = urlLower.includes('diario') || 
                                              urlLower.includes('diário') || 
                                              urlLower.includes('oficial') ||
                                              ...
                                              urlLower.includes('doem') ||
                                              urlLower.includes('doe');
                    const hasEditionNumber = editionNum && pdfUrl.includes(editionNum);
                    const isOtherDocument = urlLower.includes('diagnostico') ||
                                           urlLower.includes('diagnóstico') ||
                                           urlLower.includes('tecnico') ||
                                           urlLower.includes('técnico') ||
