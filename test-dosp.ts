/**
 * Test script for DOSP spiders
 * 
 * Usage: npx tsx test-dosp.ts [spider_id]
 * 
 * Examples:
 *   npx tsx test-dosp.ts ce_horizonte
 *   npx tsx test-dosp.ts mg_itajuba
 *   npx tsx test-dosp.ts ms_deodapolis
 */
...
async function main() {
  ...
  if (!spiderId) {
    console.log('Available DOSP spiders:');
    const dospConfigs = spiderRegistry.getConfigsByType('dosp');
    
    for (const config of dospConfigs) {
