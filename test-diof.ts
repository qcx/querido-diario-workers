/**
 * Test script for DIOF spiders
 * 
 * Usage: npx tsx test-diof.ts [spider_id]
 * 
 * Examples:
 *   npx tsx test-diof.ts al_igaci
 *   npx tsx test-diof.ts ba_abare
 *   npx tsx test-diof.ts ba_adustina
 */
...
async function main() {
  ...
  if (!spiderId) {
    console.log('Available DIOF spiders:');
    const diofConfigs = spiderRegistry.getConfigsByType('diof');
    
    for (const config of diofConfigs) {
