/**
 * Test script for Instar spiders
 * 
 * Usage: npx tsx test-instar.ts [spider_id]
 * 
 * Examples:
 *   npx tsx test-instar.ts mg_betim
 *   npx tsx test-instar.ts mg_campo_belo
 *   npx tsx test-instar.ts mg_candeias
 */
...
async function main() {
  ...
  if (!spiderId) {
    console.log('Available Instar spiders:');
    const instarConfigs = spiderRegistry.getConfigsByType('instar');
    
    for (const config of instarConfigs) {
